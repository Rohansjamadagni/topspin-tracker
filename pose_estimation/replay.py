...
parser.add_argument('-i', '--input', help="Enter path to directory obtained from record.py", default=get_path_name())
...
try:
	...
	def create_pipeline():
		...
		pose_nn.setBlobPath(str(Path("/home/rohanj/Documents/topspin/models/pose.blob").resolve().absolute()))
	...
	def main():
		...
		for file_ in os.listdir(args.input):
			if os.path.isdir(args.input+'/'+file_):
				valid_frames.append(file_)
		...
		with dai.Device(create_pipeline()) as device:
			...
			for frame in frames:
				...
				progress = frame/frames[-1]*100
            	# publish progress every 5% 
				if int(frame) % int(0.05*frames[-1]) == 0:
			...
			for frame in range(0, num_skipped_frames):
				...
				if int(frame) % 60 == 0:
					client.publish("topspin/test", "Pose Estimator Processing...")
